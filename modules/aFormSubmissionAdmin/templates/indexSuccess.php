  ...
  <?php include_partial('aFormSubmissionAdmin/list_bar', array('filters' => $filters)) ?>
	...
	<div id="a-admin-subnav" class="subnav">
		...
		<ul class="a-controls a-admin-action-controls">
			  			<li class="filters"><?php echo jq_link_to_function("Filters", "$('#a-admin-filters-container').slideToggle()" ,array('class' => 'a-btn icon a-settings', 'title'=>'Filter Data')) ?></li>
							<li><?php include_partial('aFormSubmissionAdmin/list_header', array('pager' => $pager)) ?></li>
		</ul>
  </div>
	...
	<div id="a-admin-content" class="main">
		...
		<?php include_partial('aFormSubmissionAdmin/flashes') ?>
				...
				<?php include_partial('aFormSubmissionAdmin/list', array('pager' => $pager, 'sort' => $sort, 'helper' => $helper, 'a_form' => $a_form)) ?>
